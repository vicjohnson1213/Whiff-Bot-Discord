...
const settingsTemplate = require('./templates/settings.json');
...
function loadAllSettings(guildIds) {
    guildIds.forEach((id) => {
        ...
        if (!fs.existsSync(settingsPath)) {
            fs.writeFileSync(settingsPath, JSON.stringify(settingsTemplate, ' ', 2));
            guildSettings[id] = _.cloneDeep(settingsTemplate);
        } else {
