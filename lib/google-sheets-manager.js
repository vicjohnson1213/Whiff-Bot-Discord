...
function getNoAFKUsers() {
    return new Promise((resolve, reject) => {
        doc.getRows(NO_AFK_SHEET, (err, rows) => {
            const users = {};
            rows.forEach(r => {
                if (users[r.guildid]) {
                    users[r.guildid].push(r.userid);
                } else {
                    users[r.guildid] = [r.userid];
                }
            });
